...
function run_variance_reduction_tests(num_paths=1000, seed=42)
    ...
    bs_exact_results = analyze_antithetic_variance_reduction(bs_prob, bs_exact_method; 
    ...
    bs_em_results = analyze_antithetic_variance_reduction(bs_prob, bs_em_method; 
                                                         num_paths=num_paths, seed=seed)
    ...
    heston_em_results = analyze_antithetic_variance_reduction(heston_prob, heston_em_method; 
                                                             num_paths=num_paths, seed=seed)
    ...
    heston_bk_results = analyze_antithetic_variance_reduction(heston_prob, heston_bk_method; 
                                                             num_paths=bk_paths, seed=seed)
    
    print_results(heston_bk_results)
    
    println("\n=== Summary of Variance Reduction Factors ===")
    println("1. Black-Scholes (Exact):      $(round(bs_exact_results.var_reduction_ratio, digits=2))×")
    println("2. Black-Scholes (Euler-M):    $(round(bs_em_results.var_reduction_ratio, digits=2))×")
    println("3. Heston (Euler-M):           $(round(heston_em_results.var_reduction_ratio, digits=2))×")
    println("4. Heston (Broadie-Kaya):      $(round(heston_bk_results.var_reduction_ratio, digits=2))×")
end
