...
function plot_against_strike(plot_func, fun, pricer, strike_range)
	...
	return plot_func(strike_range, s -> fun(pricer, s, pricer.marketInputs.sigma), markercolor=markercolor, markersize=1, markerstrokewidth=0, label=string(nameof(fun)), xlabel="Strike", ylabel="Price")	
end
...
function plot_against_vol(plot_func, fun, pricer, vol_range)
	...
	return plot_func(vol_range, v -> fun(pricer, pricer.payoff.strike, v), markercolor=markercolor, markersize=1, markerstrokewidth=0, label=string(nameof(fun)), xlabel="Vol", ylabel="Price")	
end
